(function (nx, ui, toolkit, annotation, global) {
    var BOUND = 2;
    var DEFAULT_AXIS = "y";
    var EXPORT = nx.define("nxex.common.Scroller", nxex.struct.Element, {
        struct: {
            properties: {
                class: "scroller"
            },
            ...
            events: {
                mousedown: function (sender, evt) {
                    evt.captureDrag(this);
                },
                dragstart: "handler_dragstart",
                dragmove: "handler_dragmove",
        ...
        methods: {
             ...
             */
            update: function () {
                var inBound = this.inner().dom().getBound();
                var outBound = this.dom().getBound();
                this._in = {
                    x: inBound.width,
                    y: inBound.height
                };
            ...
            animate: function (seconds, bezier) {
                if (seconds === false) {
                    this.inner().dom().removeStyle("transition-duration");
                    this.inner().dom().removeStyle("-moz-transition-duration");
                    this.inner().dom().removeStyle("-o-transition-duration");
                    this.inner().dom().removeStyle("-webkit-transition-duration");
                    this.inner().dom().removeStyle("transition-timing-function");
                    this.inner().dom().removeStyle("-moz-transition-timing-function");
                    this.inner().dom().removeStyle("-o-transition-timing-function");
                    this.inner().dom().removeStyle("-webkit-transition-timing-function");
                } else {
                    var val = seconds + "s";
                    this.inner().dom().setStyles({
                        ...
                        "-moz-transition-timing-function": "cubic-bezier" + bezier,
                        "-o-transition-timing-function": "cubic-bezier" + bezier,
                        "-webkit-transition-timing-function": "cubic-bezier" + bezier
                    });
                }
            }
        },
