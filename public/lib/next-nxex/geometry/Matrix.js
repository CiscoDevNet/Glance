(function (nx, ui, toolkit, annotation, global) {
     ...
     */
    var EXPORT = nx.define("nxex.geometry.Matrix", nx.Observable, {
        mixins: [nxex.geometry.MatrixSupport],
        methods: {
            init: function (matrix) {
                this.inherited();
                this.matrix(matrix ? toolkit.clone(matrix) : EXPORT.I);
        ...
        statics: {
            ...
            approximate: function (m1, m2, precision) {
                ...
                for (i = 0; i < m1.length; i++) {
                    if (!nxex.geometry.Vector.approximate(m1[i], m2[i], precision)) {
                        return false;
                    }
                }
                return true;
            ...
            equal: function (m1, m2) {
                ...
                var i;
                for (i = 0; i < m1.length; i++) {
                    if (!nxex.geometry.Vector.equal(m1[i], m2[i])) {
                        return false;
                    }
                }
                return true;
            ...
            inverse: function (m) {
                ...
                return [[d / deno, -b / deno, 0], [-c / deno, a / deno, 0], [(c * f - d * e) / deno, (b * e - a * f) / deno, 1]];
            }
        }
    });
